...
/* server ÖÐ ÐèÒªÊµÏÖ¶ÔÓ¦µÄ»Øµ÷º¯Êý */
...
class SIP_SERVER:public SIP_BASIC, public GBCallBack
{
	public :
		...
		/* Éè±¸×¢²áÊ± ÐèÒª¶ÔÓ¦µÄdeviceID ¾ùÆ¥Åä */
...
void SIP_SERVER::RemoveDevice(int index)
{
	...
	/* Èç¹ûÊÇ×îºóÒ»¸öÉè±¸£¬Ö±½ÓÉ¾³ý£¬Èç¹û²»ÊÇ×îºóÒ»¸ö£¬½«ÏÖÓÐµÄÉ¾³ýµô£¬ºóÃæµÄÇ°ÒÆ¾Í¿ÉÒÔ */
...
void  SIP_SERVER::DeviceManage()
{
	...
	while(1)
	{
		...
		sleep(1);/* linuxÏÂ µ÷ÓÃsleep µ¥Î»ÊÇÃë¼¶ */
...
char*replace(char*src, char*sub, char*dst)
{
    ...
    subLen = strlen(sub);
    dstLen = strlen(dst);
    pRet = (char*)malloc(srcLen + dstLen - subLen +1);//(å¤–éƒ¨æ˜¯å¦è¯¥ç©ºé—´)if (NULL != pRet)
    {
...
int SIP_SERVER::Call_Build_Initial_Invite(int index,const char * rtp_svr, int rtp_svr_port)
{
		...
		/*100relÊÇÁÙÊ±ÏìÓ¦µÄÈ·ÈÏ»úÖÆ, 1xxÁÙÊ±ÏìÓ¦£¬ÒªÇó¶Ô¶ËÊ¹ÓÃPRACKÐÅÁîÈ·ÈÏÊÕµ½±¾ÐÅÁî
		ÊÕµ½PRACKºó£¬±¾¶ËÕë¶ÔPRACKÏàÓ¦200OK£¬±ãÈçÈý´ÎÎÕÊÖ*/
		...
		{  
			...
			eXosip_lock();
			eXosip_execute();
			eXosip_automatic_action (); //²¿·Önon-200ÏûÏ¢×Ô¶¯ÖØ·¢£¬SIP»á»°ÖÐRetryºÜ³£¼û
			eXosip_automatic_refresh();/*Refresh REGISTER and SUBSCRIBE before the expiration delay*/	
			eXosip_unlock();
			if(je == NULL)  
			{  
	...
	else //Test and Verify
	{
		...
		if (!strcmp(Response, response_h))
		{
			...
			if(-1 == index)
			{
				...
				/* ÐèÒªÖØ¸´¸²¸ÇÊ±£¬ÐèÒª×¢Òâ½«Ô­ÓÐµÄ¶ÔÏóÊÍ·Åµô */
			...
			{
				/* ÒÑ¾­×¢²áµÄÇé¿öÏÂ²»ÔÙÔö¼ÓÉè±¸ ¸üÐÂÆäËûÐÅÏ¢  ÃÜÂë video ¶Ë¿Ú ÒÔ¼°expire */
				//this->ipc_list[this->m_ipc_num]->m_videoport = VIDEOPORT;	
...
{
		osip_message_t* asw_invite= NULL;/*ÇëÇóµÄÈ·ÈÏÐÍÓ¦´ð*/
		...
		"o=32032200001120000001 0 0 IN IP4 221.226.150.236\r\n"/*»á»°Ô´*//*ÓÃ»§Ãû/»á»°ID/°æ±¾/ÍøÂçÀàÐÍ/µØÖ·ÀàÐÍ/µØÖ·*/
		"s=Embedded IPC\r\n"/*»á»°Ãû*/
		"c=IN IP4 221.226.150.236\r\n"/*Á¬½ÓÐÅÏ¢*//*ÍøÂçÀàÐÍ/µØÖ·ÐÅÏ¢/¶àµã»áÒéµÄµØÖ·*/
		"t=0 0\r\n"/*Ê±¼ä*//*¿ªÊ¼Ê±¼ä/½áÊøÊ±¼ä*/
		"m=video %d RTP/AVP 96\r\n"/*Ã½Ìå/¶Ë¿Ú/´«ËÍ²ãÐ­Òé/¸ñÊ½ÁÐ±í*/
		"a=sendonly\r\n"/*ÊÕ·¢Ä£Ê½*/
		"a=rtpmap:96 PS/90000\r\n"/*¾»ºÉÀàÐÍ/±àÂëÃû/Ê±ÖÓËÙÂÊ*/
...
void    SIP_SERVER::ProcessRTPDataToPS(int * p_port)
{
		...
		while(1)
		{
			...
			r=recvfrom(fd, buf, sizeof(buf), 0, (struct sockaddr*)&from, &len);//³É¹¦Ôò·µ»Ø½ÓÊÕµ½µÄ×Ö·ûÊý,Ê§°Ü·µ»Ø-1.
...
{  
	...
	{
		printf("error£ºCreateThread failed£¡\n");
		return -1;
	}
	/* ¶ÔÓ¦µÄÏß³ÌÆðÀ´ */
